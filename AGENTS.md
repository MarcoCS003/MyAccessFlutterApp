...
- **No ACK (sync v2)**: FCM is just the messenger; the client never confirms delivery (`POST /notifications/ack` was removed app-side; `delivered_at` is dead). Reconciliation is client-driven via `POST /notifications/diff` (the client sends its latest 10 local `backendId`s and the backend returns only the missing rows).
- **Multi-session background sync (v2)**: `notification_sync_task.dart` iterates ALL saved sessions, each with its own JWT, only inside the off-peak windows **9:00–10:00 and 15:00–16:00** (+30 min grace). Each device syncs at a random persisted slot `settingsBox['deviceSyncSlot']` (0–3599 s; fallback `user_id % 3600`). Per-account per-window markers `lastDiffSync_<userKey>` (`yyyy-MM-dd:HH`) prevent duplicate runs; a failed account keeps its marker unset and retries next window. Scheduler: one-off WorkManager re-scheduled after each run (`scheduleNextSyncWindow` in `background_sync_register.dart`), iOS uses `background_fetch`. `syncNow()` (pull-to-refresh / QA) bypasses window checks; `maybeSyncOnAppOpen()` syncs only if the current/last started window lacks a marker. Window math lives in `lib/features/notifications/background/sync_window.dart`.
...
- **Backend messages include a `notification` key** and, since firebase_messaging **15.x** (`FlutterFirebaseMessagingReceiver` enqueues the background isolate for EVERY background message, even with a `notification` block), `onBackgroundMessage` DOES fire in background: it persists to Hive but must NOT show the local tray when `message.notification != null` (the system already shows its own tray — showing both caused the duplicate notifications of 2026-09-05). The old assumption "onBackgroundMessage does NOT fire with a notification block" is obsolete for this plugin version.
...
- **Release signing (upload key de Play):** `android/app/build.gradle.kts:43-56` lee `android/key.properties` y aplica `signingConfigs.release` al `buildTypes.release`. El keystore está **fuera del repo** en `/home/marcocarrasco/.config/myaccess/myaccess-upload.jks` (alias `myaccess-upload`); el path absoluto y los passwords están en `android/key.properties` (gitignored junto a `**/*.jks` y `**/*.keystore` en `android/.gitignore:12-14`). El password del store y del secret key son el mismo — guardado en `/home/marcocarrasco/.config/myaccess/upload-password.txt`. **Perder ese .jks o el password impide actualizar la app en Play** (Google no guarda la upload key).
